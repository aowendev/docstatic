---
lastmod: '2026-09-02T06:55:36.521Z'
...
Navigieren Sie im CMS zur Sammlung „Dashboards“. Oben auf der Seite befindet sich eine Statusleiste mit vier Anzeigen:

* **GraphQL**: Gibt an, ob eine Abfrage an die API tatsächlich eine Antwort liefert. Ist dies nicht der Fall, wird hier der Grund dafür angezeigt.
* **Umgebung**: Welche API hat eine Überprüfung beantwortet – die Cloud oder der Localhost?
* **Datenquelle**: Woher beziehen die Dashboards ihre Inhalte – aus Ihren lokalen Dateien oder aus TinaCloud?
* **Einstellungen**: Gibt an, ob die vom CMS benötigte Konfiguration vorhanden und gültig ist.

**Umgebung** und **Datenquelle** beantworten unterschiedliche Fragen und können voneinander abweichen. Ersteres gibt an, welche API auf eine Abfrage reagiert hat; letzteres gibt an, wohin das CMS seine Abfragen sendet. Wenn sie voneinander abweichen, gibt **Datenquelle** an, welche Inhalte Sie gerade betrachten. Beim Ausführen des lokalen Entwicklungsservers wird das CMS anhand Ihrer lokalen Dateien erstellt. Beenden Sie den Server daher, um mit Cloud-Inhalten zu arbeiten.
