...
const Dashboard1 = () => {
  ...
  // Tracks whether the overview has been loaded at least once, so changing a
  // filter refetches but mounting does not auto-load. Kept in a ref so the
  // effect below doesn't have to depend on contentData (which the fetch sets,
  // which would loop).
  ...
  // Refetch when the activity filters change (fetchContentOverview's identity
  // changes with them), but not before the first load.
  useEffect(() => {
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
            <span className="text-3xl text-tina-orange">
              ...
              >
                ...
                <path d="M21 16.5c0-.8-.7-1.5-1.5-1.5s-1.5.7-1.5 1.5.7 1.5 1.5 1.5 1.5-.7 1.5-1.5z" />
                <path d="M3 7.5C3 6.7 3.7 6 4.5 6S6 6.7 6 7.5 5.3 9 4.5 9 3 8.3 3 7.5z" />
              </svg>
            </span>
  ...
  const StatCard = ({ title, count, total, color, percentage, onClick }) => {
    ...
    return (
      ...
      <div
        ...
        onKeyDown={(e) => {
          if (e.key === "Enter" || e.key === " ") {
            e.preventDefault();
            if (count > 0 && onClick) onClick(title);
          }
        }}
      ...
      >
        <div
          className={`${getBackgroundColor(title)} p-3 rounded-lg flex-shrink-0`}
        >
        ...
        <div className="min-w-0">
          ...
          <p className="text-xs text-gray-400">
            {Number.isNaN(percentage) ? "0" : percentage.toFixed(0)}% of {total}
          </p>
        </div>
      </div>
    );
    ...
    >
      ...
      <div style={{ marginBottom: "24px" }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
          <StatCard
            title="Draft"
            count={contentData.docs.draft || 0}
            total={contentData.docs.total}
            color="#f59e0b"
            percentage={
              ((contentData.docs.draft || 0) / contentData.docs.total) * 100 ||
              0
            }
          ...
          <StatCard
            title="Review"
            count={contentData.docs.review || 0}
            total={contentData.docs.total}
            color="#3b82f6"
            percentage={
              ((contentData.docs.review || 0) / contentData.docs.total) * 100 ||
              0
            }
            onClick={filterDocumentsByStatus}
          />
          <StatCard
            title="Translate"
            count={contentData.docs.translate || 0}
            total={contentData.docs.total}
            color="#8b5cf6"
            percentage={
              ((contentData.docs.translate || 0) / contentData.docs.total) *
                100 || 0
            }
            onClick={filterDocumentsByStatus}
          />
          <StatCard
            title="Approved"
            count={contentData.docs.approved || 0}
            total={contentData.docs.total}
            color="#10b981"
            percentage={
              ((contentData.docs.approved || 0) / contentData.docs.total) *
                100 || 0
            }
            onClick={filterDocumentsByStatus}
          />
          <StatCard
            title="Published"
            count={contentData.docs.published || 0}
            total={contentData.docs.total}
            color="#059669"
            percentage={
              ((contentData.docs.published || 0) / contentData.docs.total) *
                100 || 0
            }
            onClick={filterDocumentsByStatus}
          />
          <StatCard
            title="Unlisted"
            count={contentData.docs.unlisted || 0}
            total={contentData.docs.total}
            color="#6b7280"
            percentage={
              ((contentData.docs.unlisted || 0) / contentData.docs.total) *
                100 || 0
            }
            onClick={filterDocumentsByStatus}
          />
            ...
            contentData.recentActivity.map((item, index) => (
              <div
                ...
                style={{
                  padding: "12px 16px",
                  borderBottom:
                    index < contentData.recentActivity.length - 1
                      ? "1px solid #f6f8fa"
                      : "none",
                  display: "flex",
                  justifyContent: "space-between",
