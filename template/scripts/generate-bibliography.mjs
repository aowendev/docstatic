...
/**
 * The frontmatter keys this generator decides, and therefore rewrites on every
 * run. Anything else on the page belongs to whoever put it there.
 */
...
/** A key at the start of a line - the only place a top-level YAML key can be. */
const TOP_LEVEL_KEY = /^([A-Za-z_][\w-]*)\s*:/;

/**
 * The frontmatter lines on the existing page that this generator does not own.
 *
 * The page is generated, but it still lives in the docs collection, so saving
 * anything in its vicinity in the CMS stamps `lastmod` and `modifiedBy` onto
 * it. Rebuilding the frontmatter from a fixed list threw those away every run,
 * which showed up as the page reappearing in `git status` after every build -
 * a generated file that will not stay generated is one nobody trusts.
 *
 * Carried as raw lines rather than parsed and re-serialised. These values were
 * written by something else and only have to survive intact: parsing
 * `modifiedBy: name <email@host>` and writing it back through a YAML
 * serialiser would quote it differently and trade one phantom diff for
 * another. A line that is not a top-level key continues the key above it - a
 * list item, a nested mapping, a wrapped value - so it is carried or dropped
 * with whatever key it belongs to.
 */
export function foreignFrontmatterLines(source) {
  ...
  for (const line of lines.slice(1, end)) {
    ...
    if (match) keeping = !OWNED_FRONTMATTER_KEYS.has(match[1]);
    if (keeping) carried.push(line);
  }

  return carried;
}

function frontmatter(title, description, carried = []) {
  return [
    "---",
    ...
    // Last, and in the order they were found, so that running the generator
    // twice over its own output produces the same bytes.
    ...carried,
    "---",
  ].join("\n");
}
...
function buildPage(entries, lang, pageTitles, includeUncited, existingSource) {
  ...
  return [
    frontmatter(
      strings.title,
      strings.description,
      foreignFrontmatterLines(existingSource)
    ),
    "",
...
function main() {
  ...
  for (const lang of languages) {
    ...
    const file = pageFileFor(lang);
    const existingSource = fs.existsSync(file)
