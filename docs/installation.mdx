---
lastmod: '2026-08-19T14:49:54.079Z'
...
* <Url urlKey="github" />: Stores your content and tracks changes
* <Url urlKey="tinacms" />: Enables browser-based editing
* <Url urlKey="languagetool" />: Provides optional grammar and style checking
...
You need to install <Url urlKey="node-js" /> version 22.0 or above.
...
1. Go to the <Url urlKey="nodejs-prebuilt-installer" /> page.
...
If you created your site this way, you can skip ahead to the [Project structure](#project-structure) section.
...
* `/bin/` - The `docstatic` CLI entry point that `create-docstatic` runs to scaffold a new site. Only present if you cloned the docStatic repository.
* `/blog/` - Blog MDX files.
* `/config/` - JSON files used by TinaCMS to configure docStatic, including the chatbot and theme settings.
* `/create-docstatic/` - The npm package published as `create-docstatic`. Only present if you cloned the docStatic repository.
* `/docs/` - Docs MDX files.
* `/i18n/` - Translation files.
* `/mcp-server/` - The [MCP server](mcp-server.mdx) that gives AI assistants access to your documentation.
* `/reuse/` - Reusable content, including managed URLs and homepage and media settings.
* `/scripts/` - Build-time scripts run automatically by `prebuild` and `predev`.
* `/src/` - Non-documentation files like pages or custom React components.
  * `/src/components` - Custom React components, such as the dashboards and chatbot.
...
* `/template/` - The scaffolding template kept in sync with the main site and shipped to new sites by `create-docstatic`. Only present if you cloned the docStatic repository. See [CLI](cli.mdx#template-sync-commands).
* `/test/` - The project's test suite. Only present if you cloned the docStatic repository.
...
For more information, see <Url urlKey="docusaurus-monorepos" /> in the Docusaurus documentation.
