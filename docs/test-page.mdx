---
title: Test page
description: Page for testing out new docStatic features.
tags:
  - docs
  - docs_author
---

# Embeddable components

#### Admonitions

<Admonition type="note" title="Note Title">
  This is one of the five types of admonition. The others are `Tip`, `Info`, `Caution` and `Warning`.
</Admonition>

#### Code snippets

<CodeSnippet title="This is a code snippet from a file:" language="xml" filepath="example.xml" />

#### Comments

<!-- hidden comment="This comment will not be visible, even by inspecting the output." -->

#### Context help

[URL](#csh)

<a id="csh" />

#### Details

<Details summary="This is an expanding block.">
  Here are the details.
</Details>

#### Figures

Include captions and open the full-size image in a new window.

<Figure img="/img/docStaticDemo.jpg" caption="An image." size="100%" />

#### Footnotes

This is a sentence with a footnote<Footnote>This is the content of the first footnote. It can contain **markdown** and other formatting.</Footnote>.

Here's another sentence with a different footnote<Footnote>This is a second footnote with different content. You can include links and other elements here.</Footnote>.

And here's a reference to the same footnote again<Footnote>This is the content of the first footnote. It can contain **markdown** and other formatting.</Footnote>.

#### Glossary terms

Hover over this glossary term: <GlossaryTerm termKey="single-sourcing" />

#### Snippets

<Snippet filepath="example.mdx" />

#### Tabs

<Tabs>
  <TabItem value="apple" label="Apple">
    This is an apple 🍎
  </TabItem>

  <TabItem value="orange" label="Orange">
    This is an orange 🍊
  </TabItem>

  <TabItem value="banana" label="Banana">
    This is a banana 🍌
  </TabItem>
</Tabs>

#### Variable sets

This is a variable: <VariableSet setKey="writing-terms" variableKey="single-sourcing" />
