---
lastmod: '2026-09-02T06:55:36.266Z'
...
From the CMS, navigate to the Dashboards collection. At the bottom of the page is a status bar with four readings:

* **GraphQL**: whether a query to the API actually returns. If it does not, the reason is shown here.
* **Environment**: which API answered a check, the cloud or localhost.
* **Data source**: where the dashboards read their content from, your local files or TinaCloud.
* **Settings**: whether the configuration the CMS needs is present and valid.

**Environment** and **Data source** answer different questions and can disagree. The first reports which API responded to a check; the second reports where the CMS was built to send its queries. When they differ, **Data source** is the one that tells you which content you are looking at. Running the local dev server builds the CMS against your local files, so stop it to work with cloud content.
