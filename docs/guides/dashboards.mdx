---
lastmod: '2026-08-15T11:05:35.926Z'
title: Dashboards
...
The dashboard reports on the hyperlinks in your topics *and* the URLs stored in the URLs collection, including their HTTP status, so a page that returns 404 or 500 is identified as broken no matter which source it came from. For example, [this link](https://docusaurus.io/this-page-does-not-exist-demo-404) intentionally returns a 404, so `yarn check-links` reports it as broken.

The dashboard displays the results of the last `yarn check-links` run, bundled into the site the last time it was built. To check your links with real HTTP statuses, run:
