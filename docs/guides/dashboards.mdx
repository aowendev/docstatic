---
lastmod: '2026-08-15T09:41:49.982Z'
title: Dashboards
...
The dashboard reports on the hyperlinks in your topics *and* the URLs stored in the URLs collection, including their HTTP status, so a page that returns 404 or 500 is identified as broken no matter which source it came from.
...
This requests every external link — both hardcoded in docs and stored in the URLs collection — records what each server answered, and updates the report. Run it whenever you want fresh results. The dashboard shows the date and time of the last check.
...
* Migration candidates: distinct external URLs that are hardcoded in a doc but not yet present in the URLs collection.
