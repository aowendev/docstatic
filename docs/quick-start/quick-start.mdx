...
`Draft` → `In Review` → `In Translation` → `Translated` → `Published` ( → `Unlisted`)

Each state is a metadata Boolean, but the user interface in Tina treats them as a unified workflow.
