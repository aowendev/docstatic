...
function extractInlineCode(node, results = []) {
  ...
  if (Array.isArray(node)) {
    for (const n of node) extractInlineCode(n, results);
    return results;
  }
...
function extractParagraphTexts(node, results = []) {
  if (!node || typeof node !== "object") return results;
  if (Array.isArray(node)) {
    for (const n of node) extractParagraphTexts(n, results);
    return results;
  }
  if (node.type === "p" || node.type === "paragraph") {
    ...
    for (const c of node.children) extractParagraphTexts(c, results);
  }
  return results;
}
...
function computeSuggestions(docBodies, glossaryTermsData, variableSetsData) {
    ...
    for (const { path, body } of docBodies) {
      ...
      if (
        extractPlainText(body)
          .toLowerCase()
          .includes(term.termText.toLowerCase())
      )
        plainPaths.push(path);
    }
  ...
  for (const { path, body } of docBodies) {
    ...
    for (const para of extractParagraphTexts(body)) {
      ...
      if (!paraMap[normalized])
        paraMap[normalized] = { original: para, paths: [] };
      if (!paraMap[normalized].paths.includes(path))
        paraMap[normalized].paths.push(path);
    }
  }
...
async function fetchReuseData() {
  ...
  const variableSetsConn = await client.queries.variableSetsConnection({
    first: 100,
  });

  let snippets = [];
  if (client.queries.snippetsConnection) {
    const snippetsConn = await client.queries.snippetsConnection({
      first: 100,
    });
    snippets = (snippetsConn.data?.snippetsConnection?.edges || []).map(
      (edge) => ({
    ...
    snippets = [
      {
        name: "example.mdx",
        path: "reuse/snippets/example.mdx",
  ...
  function bodyContainsComponent(node, componentName, propName, matchFn) {
    ...
    if (Array.isArray(node))
      return node.some((n) =>
        bodyContainsComponent(n, componentName, propName, matchFn)
      );
    ...
    if (Array.isArray(node.children)) {
      return node.children.some((child) =>
        bodyContainsComponent(child, componentName, propName, matchFn)
      );
    }
    return false;
  }
  ...
  function findComponentUsage(componentName, propName, matchFn) {
    return docBodies
      .filter(({ body }) =>
        bodyContainsComponent(body, componentName, propName, matchFn)
      )
      .map(({ path }) => path);
  }

  const glossaryTermsData = (
    glossaryConn.data?.glossaryTermsConnection?.edges || []
  ).map((edge) => {
    const item = edge.node.glossaryTerms?.[0] || edge.node;
    return {
      key: item.key,
      termText:
        item.translations?.find((t) => t.lang === "en")?.term || item.key,
      definition: item.translations?.[0]?.definition || "",
      usedIn: findComponentUsage(
        "GlossaryTerm",
        "termKey",
        (val) => val === item.key
      ),
  ...
  const variableSetsData = (
    variableSetsConn.data?.variableSetsConnection?.edges || []
  ).map((edge) => {
    ...
    return {
      ...
      usedIn: findComponentUsage(
        ...
        (val) =>
          typeof val === "string" &&
          (val === item.name || val.startsWith(`${item.name}_`))
      ),
    };
  });
  ...
  return {
    codeSnippets: codeSnippets.map((item) => ({
      ...
      usedIn: findComponentUsage(
        "CodeSnippet",
        "filepath",
        (val) => typeof val === "string" && val.includes(item.name)
      ),
    })),
...
const ContentReuseDashboard = () => {
  ...
  const activeSuggestionCategory = SUGGESTION_CATEGORIES.find(
    (c) => c.type === suggestionDetailType
  );
  ...
  const renderSuggestionRows = () => {
    if (!suggestionDetailType || !reuseData.suggestions[suggestionDetailType])
      return null;
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div
                style={{
                  ...
                  marginBottom: "2px",
                  fontFamily:
                    suggestionDetailType === "codeSnippets"
                      ? "monospace"
                      : undefined,
                }}
              >
  ...
  const renderDetailPanel = (type, items, category, onClose, emptyLabel) => (
    ...
    >
      ...
      <div style={{ maxHeight: "400px", overflow: "auto" }}>
        {(items?.length ?? 0) === 0 ? (
          <div
            style={{ padding: "20px", textAlign: "center", color: "#586069" }}
          >
            No {emptyLabel} found
          </div>
        ) : type === "suggestion" ? (
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {CATEGORIES.map((cat) => {
          ...
          return (
            // biome-ignore lint/a11y/useSemanticElements: the card body is block content, which cannot legally nest inside a <button>
            <div
              key={cat.type}
              role="button"
              tabIndex={0}
              onClick={() => {
                setDetailType(detailType === cat.type ? null : cat.type);
        ...
        {SUGGESTION_CATEGORIES.map((cat) => {
          ...
          return (
            ...
            <div
              ...
              tabIndex={0}
              onClick={() => {
                setSuggestionDetailType(
                  suggestionDetailType === cat.type ? null : cat.type
                );
                setDetailType(null);
              }}
              onKeyDown={(e) => {
