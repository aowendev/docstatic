...
const GraphQLtest = () => {
  ...
  // Concurrency guard: a ref rather than `loading` state, so testConnections
  // keeps a stable identity and the effect below doesn't re-run on every toggle.
  const runningRef = useRef(false);

  const testConnections = useCallback(async () => {
    if (runningRef.current) return; // Prevent concurrent executions
    ...
    try {
      ...
      try {
        ...
        tests.push({
          ...
          details: response1.ok
            ? "Connected successfully"
            : `HTTP ${response1.status}`,
      ...
      try {
        ...
        if (!clientId) {
          tests.push({
            ...
            status: "CONFIG ERROR",
            details:
              "NEXT_PUBLIC_TINA_CLIENT_ID is not set in environment variables",
            color: "#f59e0b",
          });
        } else {
          tests.push({
      ...
      try {
        ...
        tests.push({
          ...
          status: "SECURED ✓",
          details:
            "🔒 Network blocked - Production sites properly block direct GraphQL access for security. Use TinaCMS Generated Client instead.",
          color: "#059669",
        });
      }
        ...
        if (client) {
          ...
          try {
            ...
            const sampleDocs = docs
              .slice(0, 3)
              .map((edge) => edge.node.title || edge.node._sys.filename)
              .join(", ");
