...
const getStatusIcon = (type) => {
  switch (type) {
    ...
    case "error":
      return "✗";
    case "localhost":
      // Every caller interpolates this into an HTML string, so returning JSX
      // here rendered as "[object Object]" in the status bar.
      return (
...
const StatusBar = () => {
  ...
  const checkStatus = useCallback(async () => {
    ...
    try {
      ...
      const _token = process.env.NEXT_PUBLIC_TINA_TOKEN;
      ...
      if (clientId) {
        ...
        try {
          ...
          connectionStatus = {
            type: "success",
            message: "TinaCloud configured",
          };
          environmentStatus = {
            type: "tinacloud",
            message: `TinaCloud (${clientId.substring(0, 8)}...)`,
          };
        } catch (_clientErr) {
    ...
    const _clientId = process.env.NEXT_PUBLIC_TINA_CLIENT_ID;
    const _token = process.env.NEXT_PUBLIC_TINA_TOKEN;
    ...
    try {
      const { default: docusaurusConfig } = await import(
        "../../../config/docusaurus/index.json"
      );
  ...
  useEffect(() => {
    ...
    const targetElement = document.querySelector(
      ".relative.flex-none.w-full.h-16.px-6.bg-white.border-t.border-gray-100.flex.items-center.justify-end"
    );
    if (targetElement) {
      const statusContainer = targetElement.querySelector(
        ".status-bar-injected"
      );
      if (!statusContainer) {
