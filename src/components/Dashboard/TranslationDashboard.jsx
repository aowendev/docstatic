...
const TranslationDashboard = () => {
  ...
  const handleDeleteOrphanTopics = async () => {
    ...
    try {
      ...
      const docOrphaned = translationData?.[lang]
        ...
        : [];
      const snippetOrphaned = translationData?.[lang]
        ? translationData[lang].snippets?.orphaned || []
        : [];
  ...
  const handleAddMissingTopics = async () => {
    ...
    try {
      ...
      const missingDocs = translationData?.[lang]
        ...
        : [];
      const missingSnippets = translationData?.[lang]
        ? translationData[lang].snippets?.missing || []
        : [];
      const missing = [
      ...
      for (const [idx, item] of missing.entries()) {
        ...
        if (sourceNode) {
          ...
          if (item._isSnippet) {
            ...
            if (sourceNode.modifiedBy)
              paramsBody.modifiedBy = sourceNode.modifiedBy;
            if (sourceNode.help !== undefined)
              paramsBody.help = sourceNode.help;
            if (sourceNode.slug) paramsBody.slug = sourceNode.slug;
            if (sourceNode.tags) paramsBody.tags = sourceNode.tags;
            if (sourceNode.draft !== undefined)
              paramsBody.draft = sourceNode.draft;
            if (sourceNode.review !== undefined)
              paramsBody.review = sourceNode.review;
            if (sourceNode.translate !== undefined)
              paramsBody.translate = sourceNode.translate;
            if (sourceNode.approved !== undefined)
              paramsBody.approved = sourceNode.approved;
            if (sourceNode.published !== undefined)
              paramsBody.published = sourceNode.published;
            if (sourceNode.unlisted !== undefined)
              paramsBody.unlisted = sourceNode.unlisted;
          }
        } else {
        ...
        if (sourceNode?.lastmod) {
          ...
          try {
            ...
            if (!Number.isNaN(parsed.getTime())) lastmodSource = parsed;
          } catch (_e) {
            // ignore parse errors
          }
        }
        paramsBody.lastmod = lastmodSource
          ? new Date(lastmodSource.getTime() - 1000).toISOString()
        ...
        try {
          const pct =
            missing.length > 0
              ? Math.round(((idx + 1) / missing.length) * 100)
              : 100;
          setAddProgress(pct);
          setStatus(
            `Adding ${idx + 1}/${missing.length} translations (${pct}%)`
          );
        } catch (_e) {
  ...
  const handleEditOutOfDateDoc = (file) => {
    const cleanFile = file
      ...
      .replace(/\.md$/, "");
    window.open(
      `/admin#/collections/edit/i18n/${selectedLanguage}/docusaurus-plugin-content-docs/current/${cleanFile}`,
      "_blank"
    );
  };
  ...
  const handleEditOutOfDateSnippet = (file) => {
    ...
    window.open(
      `/admin#/collections/edit/i18n/${selectedLanguage}/snippets/${cleanFile}`,
      "_blank"
    );
  };
    ...
    try {
      ...
      const _results = await xliffUtils.importXliffBundle(
        ...
        (p) => {
          if (p?.id)
            setStatus(
              `Import ${p.id}: ${p.status}${p.error ? ` - ${p.error}` : ""}`
            );
      ...
      setStatus(`Import error: ${err?.message ? err.message : String(err)}`);
      setImporting(false);
      setImportProgress(null);
    }
  };
  ...
  const scanTranslations = async () => {
    ...
    try {
      ...
      while (true) {
        const snippetsResult = await client.queries.snippetsConnection({
          first: 100,
          after: snippetsAfter,
        });
      ...
      for (const edge of i18nEdges) {
        ...
        if (after.startsWith("snippets/")) {
          ...
          snippetTranslationsByLang[lang] =
            snippetTranslationsByLang[lang] || {};
          snippetTranslationsByLang[lang][canonical] = {
            node,
            originalPath: snippetRel,
            rawPath: snippetRel,
          };
          continue;
        }
        ...
        if (cleanAfter.startsWith("api/") || cleanAfter.startsWith("wiki/"))
          continue;
        ...
        if (translationsMap[canonical]) {
          ...
          const isExistingIndex = /(?:^|\/)index$|(?:^|\/)readme$/i.test(
            existing.originalPath || ""
          );
          const isNewIndex = /(?:^|\/)index$|(?:^|\/)readme$/i.test(cleanAfter);
          if (isExistingIndex && !isNewIndex) {
            translationsMap[canonical] = {
              node,
              originalPath: cleanAfter,
              rawPath,
            };
          } else {
            try {
              const existingDate = existing.node?.lastmod
                ? new Date(existing.node.lastmod)
                : null;
      ...
      for (const key of sourceKeys) {
        ...
        missingAll.push({
          file: `docs/${key}`,
          sourceLastMod: src.lastmod || "No date",
          title,
          sourceNode: src,
        });
      }
      for (const lang of Object.keys(translationsByLang)) {
          ...
          if (!srcDate && !trDate) {
            ...
            if (trDate >= srcDate) {
              ...
              outdated.push({
                ...
                translationLastMod: tr.lastmod,
                title,
                daysBehind: Math.ceil(
                  (srcDate - trDate) / (1000 * 60 * 60 * 24)
                ),
              });
            }
          }
        ...
        for (const key of translationKeys) {
          if (!sourceKeys.has(key)) {
            ...
            orphaned.push({
              file: `${lang}/docusaurus-plugin-content-docs/current/${orig}`,
              translationLastMod: node.lastmod || "No date",
              title: node.title || orig,
            });
          }
        }
      ...
      for (const lang of Object.keys(results)) {
        ...
        for (const key of snTrKeys) {
          if (!snippetSourceKeys.has(key)) {
            ...
            const orig = entry.rawPath || entry.originalPath || key;
            snOrphaned.push({
              file: `${lang}/snippets/${orig}`,
              translationLastMod: node.lastmod || "No date",
              title: node.title || orig,
            });
          }
        }
        results[lang].snippets = {
          missing: snMissing,
          outdated: snOutdated,
          upToDate: snUpToDate,
          orphaned: snOrphaned,
  ...
  const getTotalCounts = () => {
    ...
    return Object.keys(translationData).reduce((acc, lang) => {
      ...
      acc[lang] = {
        total:
          (data.total ||
            data.missing.length + data.outdated.length + data.upToDate.length) +
          (sn.total || 0),
  ...
  if (!translationData) {
    return (
      ...
      >
        <div
          style={{ textAlign: "center", color: "#656d76", fontSize: "16px" }}
        >
    ...
    >
      ...
      >
        ...
        <div className="flex gap-4 items-center">
          ...
          <button
            ...
            onClick={async () => {
              ...
              try {
                ...
                const text = await xliffUtils.exportOutOfDateAsXliff(
                  client,
                  selectedLanguage
                );
          ...
          <input
            type="file"
            accept=".xlf,.xliff,application/xml,text/xml"
            id="xliff-upload"
            style={{ display: "none" }}
          />
          <button
      ...
      {status && (
        <div style={{ color: "green", marginBottom: "1rem" }}>{status}</div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {(() => {
          ...
          return [
            ...
            <div
              key={index}
              className="bg-white rounded-xl border border-gray-200 shadow-sm flex items-center gap-4 p-6"
            >
        ...
        <div className="flex items-center gap-4">
          ...
          >
            {languages.map((lang) => {
              ...
              const label = configEntry
                ? `${lang.toUpperCase()} - ${configEntry.label}`
                : lang.toUpperCase();
              return (
                ...
                {data.snippets.missing.length > 0 && (
                  <div style={{ marginBottom: "25px" }}>
                    ...
                    >
                      ...
                      Missing Snippet Translations (
                      {data.snippets.missing.length})
                    </h4>
                    <div
                      style={{
                        backgroundColor: "#ffebee",
                      ...
                      {data.snippets.missing.map((item, index) => (
                        <div
                          ...
                          style={{
                            ...
                            padding: "8px 0",
                            borderBottom:
                              index < data.snippets.missing.length - 1
                                ? "1px solid #ffcdd2"
                                : "none",
                          }}
                        >
                ...
                {data.snippets.outdated.length > 0 && (
                  <div style={{ marginBottom: "25px" }}>
                    ...
                    >
                      ...
                      Outdated Snippet Translations (
                      {data.snippets.outdated.length})
                    </h4>
                    <div
                      style={{
                        backgroundColor: "#fff3e0",
                      ...
                      {data.snippets.outdated.map((item, index) => (
                        <div
                          ...
                          style={{
                            ...
                            flexDirection: "column",
                            borderBottom:
                              index < data.snippets.outdated.length - 1
                                ? "1px solid #ffcc02"
                                : "none",
                            padding: "4px 0",
                          }}
                        >
                          ...
                          >
                            ...
                            <button
                              type="button"
                              onClick={() =>
                                handleEditOutOfDateSnippet(item.file)
                              }
                              style={{
                                padding: "3px 8px",
                                backgroundColor: "#f59e0b",
                ...
                {data.snippets.upToDate.length > 0 && (
                  <div style={{ marginBottom: "25px" }}>
                    ...
                    >
                      ...
                      Up to Date Snippet Translations (
                      {data.snippets.upToDate.length})
                    </h4>
                    <div
                      style={{
                        backgroundColor: "#e8f5e8",
                      ...
                      {data.snippets.upToDate.map((item, index) => (
                        <div
                          ...
                          style={{
                            ...
                            padding: "6px 0",
                            borderBottom:
                              index < data.snippets.upToDate.length - 1
                                ? "1px solid #c3e6c3"
                                : "none",
                          }}
                        >
                      ...
                      {data.snippets.orphaned.map((item, index) => (
                        <div
                          ...
                          style={{
                            ...
                            padding: "8px 0",
                            borderBottom:
                              index < data.snippets.orphaned.length - 1
                                ? "1px solid #d4c5ff"
                                : "none",
                          }}
                        >
