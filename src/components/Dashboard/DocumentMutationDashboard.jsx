  ...
  return (
    ...
    >
      ...
      <div style={{ marginBottom: "1rem" }}>
        ...
        />
        <button type="button" onClick={handleCreate} disabled={!createTitle}>
          Create
        </button>
      </div>
      <div style={{ marginBottom: "1rem" }}>
        ...
        />
        <button type="button" onClick={handleDelete} disabled={!deleteId}>
          Delete
        </button>
      </div>
      <div style={{ marginBottom: "1rem" }}>
      ...
      <div style={{ marginBottom: "1rem" }}>
        ...
        <button type="button" onClick={handleListDocuments}>
          List Documents
        </button>
        {documents.length > 0 && (
          <ul style={{ marginTop: "1rem" }}>
            {documents.map((doc) => (
              <li key={doc.id}>
                {doc.title} <span style={{ color: "#888" }}>({doc.id})</span>
              </li>
            ))}
          </ul>
        )}
      ...
      {status && (
        <div style={{ color: "green", marginTop: "1rem" }}>{status}</div>
      )}
    </div>
  );
};
