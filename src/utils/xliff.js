...
function stripControlChars(s) {
  ...
  try {
    ...
    return (
      String(s)
        .replace(/\uFEFF/g, "")
        // biome-ignore lint/suspicious/noControlCharactersInRegex: stripping control characters is the entire point of this function
...
function angleToMarker(source) {
  ...
  source = (function scanChildrenProps(s) {
    ...
    while (true) {
      ...
      const withoutChildren = fullTag
        .replace(/\bchildren=\{[\s\S]*?\}/, "")
        .replace(/\s+/g, " ")
        .trim();
      const propsStr = withoutChildren
        .replace(/^<[^\s]+/, "")
        .replace(/^[\s>]+|[\s>]+$/g, "")
        .trim();
  ...
  source = source.replace(
    /<([A-Z][\w-]*)\b([^>]*)\bchildren=\{([\s\S]*?)\}([^>]*)\/?>/g,
    (_m, name, before, child, after) => {
  ...
  function consumeList(startIdx, ordered, sourceLines) {
    ...
    while (idx < src.length) {
      ...
      while (idx < src.length) {
        ...
        const nestedUlMatch = !nestedOlMatch
          ? next.match(/^\s{2,}[*\-+]\s+(.*)$/)
          : null;
...
function serializeRichTextToMarkdown(node) {
  ...
  if (Array.isArray(node))
    return node.map(serializeRichTextToMarkdown).join("");
  ...
  switch (type) {
    ...
    case "code_block": {
      ...
      const content =
        node.value ||
        (node.children || [])
          .map((c) => serializeRichTextToMarkdown(c))
          .join("\n");
      return `\n\n\`\`\`${lang ? ` ${lang}` : ""}\n${content}\n\`\`\`\n\n`;
    }
    case "code_line": {
    ...
    case "a": {
      const text =
        (node.children || []).map(serializeRichTextToMarkdown).join("") ||
        node.title ||
        "";
    ...
    case "mdxJsxTextElement": {
      ...
      const propsToString = (pairs) => {
        ...
        return pairs
          .map((attr) => {
            ...
            const value = Object.hasOwn(attr, "value")
              ? attr.value
              : attr.val || attr.v;
            if (!name) return "";
      ...
      const deepSerialize = (obj) => {
        ...
        if (typeof obj === "object") {
          ...
          // common text fields
          if (obj.value || obj.text || obj.literal)
            return String(obj.value || obj.text || obj.literal);
      ...
      if (node.children?.length) {
        ...
        const childAttr = node.attributes.find(
          (a) =>
            a.name === "children" && a.value !== undefined && a.value !== null
        );
        if (childAttr) {
          children =
            typeof childAttr.value === "string"
              ? childAttr.value
              : deepSerialize(childAttr.value);
        }
      } else if (node.props?.children) {
        children =
          typeof node.props.children === "string"
            ? node.props.children
            : deepSerialize(node.props.children);
      } else if (node._values?.children) {
        children =
          typeof node._values.children === "string"
            ? node._values.children
            : deepSerialize(node._values.children);
      } else {
      ...
      if (
        ...
        attrPairs = node.attributes
          .filter((a) => a?.name && String(a.name).toLowerCase() !== "children")
          .map((a) => ({ name: a.name, value: a.value }));
      } else if (node.props && typeof node.props === "object") {
        attrPairs = Object.keys(node.props)
          .filter((k) => k !== "children")
          .map((k) => ({ name: k, value: node.props[k] }));
      } else if (node._values && typeof node._values === "object") {
        attrPairs = Object.keys(node._values)
          .filter((k) => k !== "children")
          .map((k) => ({ name: k, value: node._values[k] }));
      }
...
function htmlAnchorsToMarkdown(s) {
  ...
  try {
    ...
    work = _protect(work);
    work = work.replace(
      /(?<![\](<[])(?<!:\/\/)\b[A-Za-z0-9.-]+\.(com|org|net|io|dev|ai|tech|app|gov|edu)(?:\/[^(\s)]*)?/gi,
      (m) => {
    ...
    work = work.replace(
      /___MDLINK_(\d+)___/g,
      (m, idx) => _mdLinkSlots[parseInt(idx, 10)] || m
    );
    return work;
  } catch {
    return s;
...
function normalizeNestedMarkdownLinks(s) {
  ...
  try {
    return String(s).replace(
      /\[\[([^\]]+)\]\(([^)]+)\)\]\(([^)]+)\)/g,
      (_m, innerText, innerUrl, _outerUrl) => {
...
function convertJsxPropLinksToMarkdown(s) {
  ...
  try {
    ...
    out = out.replace(
      /\(jsx:([A-Z][\w-]*)((?:[^)"'{}]|"[^"]*"|'[^']*'|\{[^}]*\})*)\)([\s\S]*?)\(\/jsx:\1\)/g,
      (m, _name, props, children) => {
        const hrefMatch = String(props).match(
          /(?:href|url|to|link)=(?:"([^"]*)"|'([^']*)'|([^\s)]+))/i
        );
        if (hrefMatch) {
    ...
    out = out.replace(
      ...
      (m, _name, props) => {
        ...
        if (hrefMatch) {
          const href = hrefMatch[1] || hrefMatch[2] || hrefMatch[3] || "";
          const textMatch = String(props).match(
            /(?:title|caption|alt)=?(?:"([^"]*)"|'([^']*)'|([^\s)]+))/i
          );
          const text =
            textMatch && (textMatch[1] || textMatch[2] || textMatch[3])
    ...
    out = out.replace(/<([A-Z][\w-]*)\b([^>]*)\/>/g, (m, _name, props) => {
      const hrefMatch = String(props).match(
        /(?:href|url|to|link)=(?:"([^"]*)"|'([^']*)'|([^\s>]+))/i
      );
      if (hrefMatch) {
...
export async function exportOutOfDateAsXliff(client, language) {
  ...
  const i18nResult = await client.queries.i18nConnection({
    sort: "title",
    first: 1000,
  });
  ...
  for (const key of Object.keys(sourceMap)) {
    ...
    if (src.raw) {
      ...
      if (src._values.body && typeof src._values.body === "object") {
        try {
          sourceBody = serializeRichTextToMarkdown(src._values.body);
        } catch {
          sourceBody = JSON.stringify(src._values.body);
        }
      } else if (src._values.body && typeof src._values.body === "string") {
        sourceBody = src._values.body;
      } else if (src._values.children) {
        try {
          sourceBody = serializeRichTextToMarkdown(src._values);
        } catch {
          sourceBody = JSON.stringify(src._values);
        }
      } else {
        try {
          sourceBody = JSON.stringify(src._values);
        } catch {
          sourceBody = String(src._values);
        }
      }
    ...
    if (tr.raw) {
      ...
      if (tr._values.body && typeof tr._values.body === "object") {
        try {
          targetBody = serializeRichTextToMarkdown(tr._values.body);
        } catch {
          targetBody = JSON.stringify(tr._values.body);
        }
      } else if (tr._values.body && typeof tr._values.body === "string") {
        targetBody = tr._values.body;
      } else {
        try {
          targetBody = JSON.stringify(tr._values);
        } catch {
          targetBody = String(tr._values);
        }
      }
    ...
    const sourcePath =
      (src?._sys && (src._sys.relativePath || src._sys.filename)) ||
      (typeof key === "string" ? `docs/${key}.mdx` : "");
    units.push({
      id: key,
      sourceTitle,
      targetTitle,
  ...
  for (const u of units) {
    ...
    let safeSource = (u.sourceBody || "").toString().trim()
      ? u.sourceBody
      : "(no source content)";
    ...
    try {
      ...
      // links.
      let conv = outsideCodeFences(normalizeNestedMarkdownLinks)(
        String(safeSource)
      );
    ...
    let safeTarget = outsideCodeFences(normalizeNestedMarkdownLinks)(
      String(u.targetBody || "")
    );
    ...
    try {
      safeTarget = outsideCodeFences((s) =>
        s.replace(/&lt;/g, "<").replace(/&gt;/g, ">")
      )(safeTarget);
...
export async function importXliffBundle(
  client,
  xliffText,
  language,
  onProgress
  ...
  if (!units || units.length === 0) {
    ...
    const segs = allEls.filter(
      (el) => (el.localName || el.tagName || "").toLowerCase() === "segment"
    );
    units = segs.map((s) => {
  ...
  for (const unit of units) {
    ...
    let id = null;
    if (unit.getAttribute?.("id")) id = unit.getAttribute("id");
    ...
    const notes = unit.getElementsByTagName
      ? Array.from(unit.getElementsByTagName("note"))
      : [];
    ...
    try {
      ...
      for (let fp = 0; fp < fenceParts.length; fp++) {
        if (fp % 2 === 0) {
          ...
          fenceParts[fp] = fenceParts[fp].replace(
            /<([A-Z][\w-]*)\b([^>]*)>([\s\S]*?)<\/\1>/g,
            "(jsx:$1$2)$3(/jsx:$1)"
          );
    ...
    if (!rel) {
      ...
      if (onProgress)
        onProgress({ id: id || null, status: "error", error: errMsg });
      continue;
    }
    ...
    try {
      ...
      try {
        ...
        if (notes?.length) {
          for (const n of notes) {
            ...
            if (lk === "path") {
              continue;
            } // path is used for ID resolution, not a mutation field
            if (lk === "title") {
              noteMeta.title = vRaw;
              continue;
            }
            if (lk === "description") {
              noteMeta.description = vRaw;
              continue;
            }
            if (lk === "tags") {
              let vals = null;
              try {
                if (/^[[{]/.test(vRaw)) vals = JSON.parse(vRaw);
              } catch {
                vals = null;
              }
      ...
      try {
        ...
        if (rawPathFromNote) {
          ...
          rp = rp.replace(
            /^[a-z]{2,3}(?:-[a-zA-Z]{2,4})?\/(?:docusaurus-plugin-content-docs\/current\/)?/,
            ""
          );
          sourceDocRel = rp;
        } else if (id) {
        ...
        if (sourceDocRel) {
          ...
          try {
            ...
            const sdoc = docQuery?.data?.doc ? docQuery.data.doc : null;
            if (sdoc) {
              ...
              for (const k of Object.keys(sdoc)) {
                ...
                if (sdoc[k] != null) {
                  sourceMetaParams[k] = Array.isArray(sdoc[k])
                    ? sdoc[k].slice()
                    : sdoc[k];
                }
              }
      ...
      try {
        ...
        try {
          ...
          existingTranslation = trQuery?.data?.i18n ? trQuery.data.i18n : null;
        } catch {
        ...
        if (existingTranslation) {
          ...
          for (const k of Object.keys(existingTranslation)) {
            ...
            if (existingTranslation[k] != null) {
              paramsObj[k] = Array.isArray(existingTranslation[k])
                ? existingTranslation[k].slice()
                : existingTranslation[k];
            }
          }
        } else {
      ...
      const respErrors = (resp && (resp.errors || resp.data?.errors)) || null;
      if (respErrors?.length) {
        const msg = Array.isArray(respErrors)
          ? respErrors
        ...
        if (onProgress)
          onProgress({ id, status: "error", error: msg, response: resp });
        continue;
      }
      ...
      const updated =
        data &&
        (data.updateI18n ||
          (data.updateI18n === null
            ? null
            : Object.values(data).find((v) => v?.id)));
      if (!updated) {
