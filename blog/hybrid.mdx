...
lastmod: '2026-08-15T13:37:30.962Z'
authors:
  - name: "Andrew Owen"
...
I couldn't figure out a way to enable the versioning and localization widgets in the nav bar with the Tina solution, so I reverted to using a fixed definition for those. For most other settings, Tina uses JSON files that are referenced by the config. Adding <Url urlKey="mermaid" /> and <Url urlKey="lunr" /> search support was trivial, although search only works in production. Rather than try to talk you through everything I had to do to get it working, this time I thought I'd simply share the [repo](https://github.com/aowendev/docstatic). The Palo Alto Networks component is made available under the MIT license, so I've used the same license. The only config you'll need to do is to add your Tina credentials to enable online web editing.
