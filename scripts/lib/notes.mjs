...
export function collectNotes(tree, { noteStyle = false } = {}) {
  ...
  const walk = (node, enclosingNote) => {
    ...
    if (isJsxElement(node) && node.name === CITE_TAG) {
      ...
      const items = readCiteItems(node);
      const signature = citationSignature(items);
...
/**
 * The fields that decide what a citation renders as, in a fixed order.
 *
 * Anything not here is either bookkeeping (Tina's _template) or does not affect
 * the rendered result, and must be excluded or an irrelevant edit would change
 * the identity of a citation that looks the same on the page.
 */
function normaliseItems(items) {
  return (Array.isArray(items) ? items : []).map((item) => [
    ...
    item?.prefix ?? "",
    item?.suffix ?? "",
    item?.suppressAuthor ? 1 : 0,
  ]);
}

/** A short content hash of a citation's items. */
export function citationSignature(items) {
  return createHash("sha1")
    .update(JSON.stringify(normaliseItems(items)))
    .digest("hex")
    .slice(0, 12);
}

/**
 * The id the compiled page carries in place of the citation text.
 *
 * Deliberately a content signature rather than a position. With positional ids
 * ("page#3"), inserting a citation earlier in a page shifts every id after it,
 * so a citations.json generated before that edit would render the *wrong
 * source* under each marker. A signature is unaffected by unrelated insertions,
 * so the worst a stale file can do is fail to resolve.
 *
 * The occurrence index keeps two identical citations on one page distinct -
 * which they must be, because the second is the one a note style shortens.
 */
export function citationId(pageKey, entry) {
  return `${pageKey}#${entry.signature}#${entry.occurrence}`;
}
