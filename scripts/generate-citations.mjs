...
import { citeEntries, collectNotes, parseMdx } from "./lib/notes.mjs";
...
function renderPage({ source, items, styleXml, locale, noteStyle, key }) {
  ...
  return {
    ...
    clusters: cites.map((entry, index) => ({
      ...
      // Stored as markdown, not citeproc's HTML: the remark plugin splices this
      // back into an mdast tree, so a parse is the natural way in and a second
      // HTML-to-JSX translation would only be a way to get it wrong.
